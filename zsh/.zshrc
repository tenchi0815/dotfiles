...
function prompt {
    ...
    if [[ $TMUX ]]; then
        ...
        usr_host="${fg_green}%n@%M%f:"
    fi

    #prompt="${usr_host}${fg_cyan}%10<...<%~%<< ${fg_base}%# "
    prompt="${usr_host}${fg_cyan}%(5~|%-1~/…/%3~|%${maxlength}>..>%~%>>) ${fg_base}%# "
    echo "${prompt}%f"
}

function git-prompt {
    echo "${fg_base}%~${fg_yellow}"'$(__git_ps1)%f '
}
